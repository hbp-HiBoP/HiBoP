...
namespace HBP.Tests.Serialization
{
    public class TargetedValidationTests
    {
        ...
        public void MergedRequests_KeepIndependentAspectScopes()
        {
            ...
            Patient patient = new();
            IEEGDataInfo first = CreateIEEG("first", protocol, patient);
            IEEGDataInfo second = CreateIEEG("second", protocol, patient);
            ValidationRequest request = new ValidationRequest(ValidationAspect.Epoching, dataInfoIDs: new[] { first.ID }).Merge(new ValidationRequest(ValidationAspect.ChannelMapping, dataInfoIDs: new[] { second.ID }));
        ...
        {
            Protocol firstProtocol = CreateProtocol("first-protocol", 10, 20);
            Protocol secondProtocol = CreateProtocol("second-protocol", 30, 40);
            Patient patient = new();
            IEEGDataInfo first = CreateIEEG("first", firstProtocol, patient);
            IEEGDataInfo second = CreateIEEG("second", secondProtocol, patient);
            ...
            DataInfoValidationResult result = await new DataInfoValidator(reader).ValidateAsync(new DataInfo[] { first, second }, request, 2, CancellationToken.None);
        ...
        {
            Protocol protocol = CreateProtocol("protocol", 10, 20);
            IEEGDataInfo dataInfo = CreateIEEG("data", protocol, new Patient());
            CountingMetadataReader reader = new(new[] { 11, 21 }, Array.Empty<string>());
            ValidationRequest request = new(ValidationAspect.Epoching, dataInfoIDs: new[] { dataInfo.ID }, force: true);

            DataInfoValidationResult result = await new DataInfoValidator(reader).ValidateAsync(new[] { dataInfo }, request, 1, CancellationToken.None);
            result.TryApply(0);
        ...
        public async Task Epoching_IgnoresMissingSecondaryEventCodes()
        {
            ...
            DataInfoValidationResult result = await new DataInfoValidator(reader).ValidateAsync(new[] { dataInfo }, new ValidationRequest(ValidationAspect.Epoching, force: true), 1, CancellationToken.None);
            result.TryApply(0);
        ...
        public async Task CcepChannelMapping_DoesNotOpenTheSource()
        {
            ...
            DataInfoValidationResult result = await new DataInfoValidator(reader).ValidateAsync(new DataInfo[] { dataInfo }, new ValidationRequest(ValidationAspect.ChannelMapping, force: true), 1, CancellationToken.None);
            result.TryApply(0);
        ...
        public void ProtocolImpact_IgnoresSecondaryCodes_AndTargetsChangedMainSubBloc()
        {
            ...
            ValidationRequest secondaryRequest = ValidationImpactAnalyzer.ForProtocols(new[] { before }, new[] { secondaryChanged });
            ...
            ValidationRequest mainRequest = ValidationImpactAnalyzer.ForProtocols(new[] { before }, new[] { mainChanged });
        ...
        public void AliasImpact_TargetsOnlyPathsWhoseResolutionChanged()
        {
            ...
            IEEGDataInfo unaffected = CreateIEEG("unaffected", protocol, new Patient());

            ValidationRequest request = ValidationImpactAnalyzer.ForAliases(new[] { new Alias("[ROOT]", "C:/old") }, new[] { new Alias("[ROOT]", "C:/new") }, new DataInfo[] { affected, unaffected }, Array.Empty<Patient>());
        ...
        public void ContainerTypeChange_InvalidatesAllDataInfoAspects()
        {
            Protocol protocol = CreateProtocol("protocol", 10, 20);
            Patient patient = new();
            IEEGDataInfo before = CreateIEEG("data", protocol, patient);
            ...
            ValidationRequest request = ValidationImpactAnalyzer.ForDataInfo(before, after);
        ...
        public void PatientImpact_IgnoresCoordinates_ButTargetsSiteRenames()
        {
            ...
            ValidationRequest coordinateRequest = ValidationImpactAnalyzer.ForPatients(new[] { before }, new[] { coordinatesChanged });
            ...
            ValidationRequest renameRequest = ValidationImpactAnalyzer.ForPatients(new[] { before }, new[] { renamed });
        ...
        public async Task PatientAssetState_CapturesFingerprint_AndRoundTrips()
        {
            ...
            PatientAssetValidationResult result = await new AssetReferenceValidator().ValidatePatientsAsync(new[] { patient }, 1, CancellationToken.None, generation: 7);
            ...
            Patient loaded = ClassLoaderSaver.LoadFromJson<Patient>(path);
        ...
        public void DataInfoValidationStates_RoundTripWithVisibleDiagnostics()
        {
            ...
            StaticDataInfo loaded = ClassLoaderSaver.LoadFromJson<StaticDataInfo>(path);
        ...
        {
            SubBloc main = CreateSubBloc($"{id}-main", MainSecondaryEnum.Main, new[] { mainCode, mainCode + 1 });
            SubBloc secondary = CreateSubBloc($"{id}-secondary", MainSecondaryEnum.Secondary, new[]
            {
        ...
        {
            HBP.Core.Data.Event mainEvent = new("main", mainCodes, MainSecondaryEnum.Main, $"{id}-main-event");
            HBP.Core.Data.Event secondaryEvent = new("secondary", new[] { 900 }, MainSecondaryEnum.Secondary, $"{id}-secondary-event");
