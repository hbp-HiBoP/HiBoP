...
namespace HBP.Tests.Serialization.LegacyNative
{
    ...
    public class Surface : CppDLLImportBase, ICloneable
    {
        ...
        public bool SearchMarsParcelFileAndUpdateColors(MarsAtlas index, string pathMarsParcel)
        {
            if (m_Backend == BenchmarkBackend.HbpCore)
            {
                if (index.Backend != BenchmarkBackend.HbpCore)
                {
        ...
        public void Append(Surface surfaceToAdd)
        {
            if (m_Backend == BenchmarkBackend.HbpCore)
            {
                if (surfaceToAdd.m_Backend != BenchmarkBackend.HbpCore)
                {
            ...
            if (surfaceToAdd.m_Backend != BenchmarkBackend.HbpExport)
            {
        ...
        public Surface Simplify(int numberOfTriangles = 10000, int agressiveness = 7)
        {
            if (m_Backend == BenchmarkBackend.HbpCore)
            {
                ...
                return new Surface(simplifiedSurface, BenchmarkBackend.HbpCore)
                {
                    IsLoaded = IsLoaded,
