...
namespace HBP.Tests.Serialization
{
    public class SerializationTypeRegistryTests
    {
        ...
        {
            Assert.That(
                SerializationTypeRegistryGenerator.TryValidate(out string error),
                Is.True,
                error);
        }
        ...
        public void EveryCurrentConcreteJsonType_IsRegistered()
        {
            Type[] missingTypes = new[]
            {
                ...
                .Where(type => !SerializationTypeRegistry.IsRegistered(type))
                .OrderBy(type => type.FullName)
                .ToArray();

            Assert.That(missingTypes, Is.Empty,
                $"Missing generated registry entries: {string.Join(", ", missingTypes.Select(type => type.FullName))}");
        }
        ...
        public void HistoricalAndCurrentNames_ResolveFromGeneratedRegistry(
            ...
            string expectedTypeName)
        {
            Type expectedType = new[]
            {
                ...
                .Distinct()
                .SelectMany(assembly => assembly.GetTypes())
                .Single(type => type.FullName == expectedTypeName);
