...
namespace HBP.Tests.Serialization
{
    public class LoadingContextTests
    {
        ...
        public void ResolveProject_BindsEveryReferenceToTheCanonicalScopedInstance()
        {
            ...
            Patient canonicalPatient = CreatePatient(
                "project patient",
                "loading-context-patient",
                new StringTagValue(placeholderTag, "value", "loading-context-value"));
            Patient placeholderPatient = CreatePatient("placeholder", canonicalPatient.ID);
            Bloc canonicalBloc = new(
                "bloc",
                0,
                string.Empty,
                string.Empty,
            ...
            Protocol canonicalProtocol = new(
                "protocol",
                new[] { canonicalBloc },
                "loading-context-protocol");
            Protocol placeholderProtocol = new(
                "placeholder",
                new[]
                {
                    new Bloc(
            ...
            Dataset dataset = new(
                ...
                "loading-context-dataset");
            Dataset placeholderDataset = new(
                "placeholder",
                placeholderProtocol,
                Array.Empty<DataInfo>(),
                dataset.ID);
            Group group = new("group", new[] { placeholderPatient }, "loading-context-group");
            IEEGColumn column = new(
                "column",
                new BaseConfiguration(),
                placeholderDataset,
                "data",
                placeholderProtocol.Blocs[0],
                new DynamicConfiguration(),
                "loading-context-column");
            Visualization visualization = new(
                "visualization",
                new[] { placeholderPatient },
                new Column[] { column },
                new VisualizationConfiguration(),
                "loading-context-visualization");

            LoadingContext context = new(
                new BaseTag[] { canonicalTag },
                new[] { canonicalProtocol },
        ...
        public void ResolveProject_GroupsMissingReferencesInOneExplicitException()
        {
            ...
            Protocol missingProtocol = new(
                "missing",
                Array.Empty<Bloc>(),
                "loading-context-missing-protocol");
            Dataset dataset = new(
                "dataset",
                missingProtocol,
                Array.Empty<DataInfo>(),
                "loading-context-invalid-dataset");
            Dataset missingDataset = new(
                "missing",
                missingProtocol,
                Array.Empty<DataInfo>(),
                "loading-context-missing-dataset");
            Group group = new("group", new[] { missingPatient }, "loading-context-invalid-group");
            StaticColumn column = new(
                "column",
                new BaseConfiguration(),
                missingDataset,
                "data",
                new StaticConfiguration(),
                "loading-context-invalid-column");
            Visualization visualization = new(
                "visualization",
                Array.Empty<Patient>(),
                new Column[] { column },
                new VisualizationConfiguration(),
                "loading-context-invalid-visualization");
            LoadingContext context = new(
                Array.Empty<BaseTag>(),
                Array.Empty<Protocol>(),
                Array.Empty<Patient>(),
            ...
            ReferenceResolutionException exception = Assert.Throws<ReferenceResolutionException>(
                () => context.ResolveProject(
                    Array.Empty<Patient>(),
                    new[] { group },
                    new[] { dataset },
        ...
        public void ResolveAuxiliaryReferences_BindsFilterTagsAndPatientConfigurations()
        {
            ...
            Patient canonicalPatient = CreatePatient(
                "canonical",
                "loading-context-configuration-patient");
            Patient placeholderPatient = CreatePatient(
                "placeholder",
                canonicalPatient.ID);
            PatientTagFilterCondition condition = new(
                PatientTagFilterCondition.TargetType.Patient,
                placeholderTag,
                new StringTagFilterValue { Value = "value" },
                false,
                "loading-context-filter-condition");
            FilterConditionsPresetCollection presets = new();
            presets.AddPreset(
                new FilterConditionsPreset(
                    "preset",
                    new BaseFilterCondition[] { condition },
                    "loading-context-filter-preset"),
                typeof(Patient),
                false);
            ...
            FilterConditionsPresetCollection loadedPresets =
                ClassLoaderSaver.LoadFromJson<FilterConditionsPresetCollection>(presetPath);
            PatientTagFilterCondition loadedCondition = (PatientTagFilterCondition)
                loadedPresets.GetPresets(typeof(Patient))[0].Conditions[0];
            PatientConfiguration configuration = new(
