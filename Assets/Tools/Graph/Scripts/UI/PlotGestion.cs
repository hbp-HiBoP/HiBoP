...
namespace Tools.Unity.Graph
{
    ...
    public class PlotGestion : MonoBehaviour
    {
        ...
        void AddCurve(CurveData curveData, Limits limits, Vector2 ratio)
        {
            ...
            {
                curve = Instantiate(ShapedCurvePrefab, transform.FindChild("Curves")).GetComponent<ShapedCurve>();
                (curve as ShapedCurve).Plot(curveData as ShapedCurveData, limits, ratio);
            }
            else
            {
                curve = Instantiate(CurvePrefab, transform.FindChild("Curves")).GetComponent<Curve>();
        ...
        void AddOriginAxes()
        {
            GameObject OrdinateOrigin = Instantiate(OriginAxePrefab, transform.FindChild("OriginAxes"));
            OrdinateOriginAxe = OrdinateOrigin.GetComponent<OriginAxe>();
            OrdinateOriginAxe.Set(OriginAxe.TypeEnum.Ordinate, OriginAxeColor);
            GameObject AbscissaOrigin = Instantiate(OriginAxePrefab, transform.FindChild("OriginAxes"));
    ...
    public class OnChangeLimitsEvent : UnityEvent<Limits>
    {
    }
}