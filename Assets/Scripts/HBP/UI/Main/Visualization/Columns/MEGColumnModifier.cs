...
namespace HBP.UI.Main
{
    public class MEGColumnModifier : SubModifier<MEGColumn>
    {
        ...
        {
            m_Protocols = ApplicationState.LoadedProject.Protocols.ToList();
        ...
        {
            m_Datasets = ApplicationState.LoadedProject.Datasets.Where((d) => d.Protocol == m_SelectedProtocol && d.GetMEGDataInfos().Length > 0 && d.GetMEGDataInfos().Any(dataInfo => dataInfo.IsOk && m_Patients.Any(p => dataInfo.Patient == p))).ToList();
