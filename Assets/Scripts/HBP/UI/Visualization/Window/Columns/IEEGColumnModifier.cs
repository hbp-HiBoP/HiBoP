...
namespace HBP.UI.Visualization
{
    public class IEEGColumnModifier : SubModifier<IEEGColumn>
    {
        ...
        void SetDataNameDropdown()
        {
            if(m_Object.Dataset != null)
            {
                m_DataNames = (from data in m_Object.Dataset.Data select data.Name).Distinct().Where((name) => m_Patients.All((patient) => m_Object.Dataset.GetIEEGDataInfos().Any((dataInfo) => dataInfo.IsOk && dataInfo.Name == name && dataInfo.Patient == patient))).ToList();
            }
            else
            {
