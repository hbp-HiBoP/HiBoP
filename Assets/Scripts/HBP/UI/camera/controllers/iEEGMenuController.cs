...
namespace HBP.VISU3D
{
    ...
    public class iEEGMenu : MonoBehaviour
    {
        ...
        public void init(Base3DScene scene, int idColumn)
        {
            m_scene = scene;
            m_columnId = idColumn;
            m_iEEGHistogram = Texture2Dutility.generate_histogram();
        ...
        private void update_histogram()
        {
            ...
            DLL.Texture.generate_distribution_histogram(m_scene.CM.IEEG_col(m_columnId).m_iEegValues, 4 * 110, 4 * 110, spanMinFactor, spanMaxFactor, middleFactor).update_texture_2D(m_iEEGHistogram);
        ...
        private void switch_ROI_menu()
        {
            ...
            //if (!m_isROIMenuMinimized)
            //    ((MP3DScene)m_scene).enableROICreationMode();
            //else
            //    ((MP3DScene)m_scene).disableROICreationMode();
