    ...
    public class SiteMenu : MonoBehaviour
    {
        ...
        void set_listeners()
        {
            ...
            m_scene.ClickSite.AddListener((idColumn) =>
            {
                ...
                if (m_scene.CM.col(idColumn).id_selected_site == -1)
                    return;
            ...
            {
                ((Column3DViewIEEG)(m_scene.CM.current_column())).sendInfos = value;
            });
        ...
        {
            m_lastSiteSelected = m_scene.CM.curr_site_of_curr_col();
            ...
            panel.Find("CCEP parent").gameObject.SetActive(m_scene.singlePatient && !m_scene.CM.is_current_column_FMRI() && !m_isCCEPMinimized);
            // text
            ...
            // active buttons            
            panel.Find("MP only options parent").Find("blacklist button").GetComponent<Button>().interactable = !m_scene.singlePatient && siteNotNull;
            ...
            panel.Find("MP only options parent").Find("load sp button").GetComponent<Button>().interactable = !m_scene.singlePatient && siteNotNull && patientComplete;
            ...
            if (m_scene.singlePatient)
            {
                ...
                if (m_scene.CM.is_current_column_FMRI())
                    return;

                Column3DViewIEEG IEEGCol = (Column3DViewIEEG)m_scene.CM.current_column();
            ...
            {
                if (m_scene.CM.curr_site_of_curr_col() != null)
                    panel.Find("MP only options parent").Find("blacklist button").Find("Text").GetComponent<Text>().text
                        = m_scene.CM.curr_site_of_curr_col().blackList ? "Unblacklist site" : "Blacklist site";
