    ...
    public class ColormapController : BothScenesOverlayController
    {
        ...
        public override void update_UI_position()
        {
            for (int ii = 0; ii < m_spColormapOverlayList.Count; ++ii)
            {
                ...
                colormap_rect_transform(true, ii).position = rectCamera.position + new Vector2(m_offsetX, rectCamera.height- m_offsetY);

                if(!m_camerasManager.get_camera(true, ii, 0).is_minimized())
                {
            ...
            for (int ii = 0; ii < m_mpColormapOverlayList.Count; ++ii)
            {
                ...
                colormap_rect_transform(false, ii).position = rectCamera.position + new Vector2(m_offsetX, rectCamera.height - m_offsetY);

                if (!m_camerasManager.get_camera(false, ii, 0).is_minimized())
                {
                    // check if enought room
        ...
        /// <returns></returns>
        public int columns_number(bool spScene)
        {
            if (spScene)
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public RectTransform colormap_rect_transform(bool spScene, int id)
        {
            if (spScene)
            {
