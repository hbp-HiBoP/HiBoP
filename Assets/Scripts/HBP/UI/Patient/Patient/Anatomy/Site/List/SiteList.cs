...
{
    public class SiteList : Tools.Unity.Lists.SelectableListWithItemAction<Site>
    {
        #region Properties
        ...
        public override bool Add(Site obj)
        {
            SortByNone();
            return base.Add(obj);
        }
        ...
        public void SortByCoordinate(Sorting sorting)
        {
            ...
            {
                case Sorting.Ascending:
                    m_Objects = m_Objects.OrderByDescending((elt) => elt.Coordinates.Count).ToList();
                    m_OrderBy = OrderBy.Coordinate;
                    m_CoordinateSortingDisplayer.Sorting = SortingDisplayer.SortingType.Ascending;
                    break;
                case Sorting.Descending:
                    m_Objects = m_Objects.OrderBy((elt) => elt.Coordinates.Count).ToList();
        ...
        public void SortByTags(Sorting sorting)
        {
            ...
            {
                case Sorting.Ascending:
                    m_Objects = m_Objects.OrderBy((elt) => elt.Tags).ToList();
                    m_OrderBy = OrderBy.Tag;
                    m_TagSortingDisplayer.Sorting = SortingDisplayer.SortingType.Ascending;
                    break;
                case Sorting.Descending:
                    m_Objects = m_Objects.OrderByDescending((elt) => elt.Tags).ToList();
