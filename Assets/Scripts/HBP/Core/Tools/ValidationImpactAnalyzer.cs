...
namespace HBP.Core.Data
{
    public static class ValidationImpactAnalyzer
    {
        public static ValidationRequest ForProtocols(IEnumerable<Protocol> before, IEnumerable<Protocol> after)
        {
            ...
            foreach (string protocolID in beforeByID.Keys.Union(afterByID.Keys, StringComparer.Ordinal))
            {
                beforeByID.TryGetValue(protocolID, out Protocol oldProtocol);
                afterByID.TryGetValue(protocolID, out Protocol newProtocol);
                Dictionary<string, string> oldSignatures = GetEpochingSignatures(oldProtocol);
                Dictionary<string, string> newSignatures = GetEpochingSignatures(newProtocol);
                IEnumerable<string> candidates = oldSignatures.Keys.Union(newSignatures.Keys, StringComparer.Ordinal);
        ...
        public static ValidationRequest ForPatients(IEnumerable<Patient> before, IEnumerable<Patient> after)
        {
            ...
            foreach (string patientID in beforeByID.Keys.Union(afterByID.Keys, StringComparer.Ordinal))
            {
                ...
                if (!string.Equals(GetAssetSignature(oldPatient), GetAssetSignature(newPatient), StringComparison.Ordinal))
                {
                    assetPatientIDs.Add(patientID);
                }

                if (!string.Equals(GetSiteNameSignature(oldPatient), GetSiteNameSignature(newPatient), StringComparison.Ordinal))
                {
                    sitePatientIDs.Add(patientID);
                }
            }
        ...
        public static ValidationRequest ForDataInfo(DataInfo before, DataInfo after)
        {
            ...
            if (before == null || before.GetType() != after.GetType() || !string.Equals(DataInfoValidationContext.GetSourceDefinitionSignature(before), DataInfoValidationContext.GetSourceDefinitionSignature(after), StringComparison.Ordinal))
            {
                return new ValidationRequest(ValidationAspect.DataInfoAll, dataInfoIDs: new[] { after.ID }, force: true);
            }
            ...
            if (!string.Equals(before.Protocol?.ID, after.Protocol?.ID, StringComparison.Ordinal))
            {
                aspects |= ValidationAspect.Epoching;
            }

            if (before is PatientDataInfo oldPatientData && after is PatientDataInfo newPatientData && !string.Equals(oldPatientData.Patient?.ID, newPatientData.Patient?.ID, StringComparison.Ordinal))
            {
                aspects |= ValidationAspect.Structure | ValidationAspect.ChannelMapping;
            }

            if (before is CCEPDataInfo oldCCEP && after is CCEPDataInfo newCCEP && !string.Equals(oldCCEP.StimulatedChannel, newCCEP.StimulatedChannel, StringComparison.Ordinal))
            {
                aspects |= ValidationAspect.ChannelMapping;
            }

            return new ValidationRequest(aspects, dataInfoIDs: new[] { after.ID }, force: true);
        }

        public static ValidationRequest ForDatasets(IEnumerable<Dataset> before, IEnumerable<Dataset> after)
        {
            Dictionary<string, DataInfo> beforeByID = (before ?? Enumerable.Empty<Dataset>()).SelectMany(dataset => dataset.Data).Where(dataInfo => dataInfo != null).GroupBy(dataInfo => dataInfo.ID, StringComparer.Ordinal).ToDictionary(group => group.Key, group => group.Last(), StringComparer.Ordinal);
            Dictionary<string, DataInfo> afterByID = (after ?? Enumerable.Empty<Dataset>()).SelectMany(dataset => dataset.Data).Where(dataInfo => dataInfo != null).GroupBy(dataInfo => dataInfo.ID, StringComparer.Ordinal).ToDictionary(group => group.Key, group => group.Last(), StringComparer.Ordinal);
            ...
            foreach (string dataInfoID in afterByID.Keys)
            {
                ...
                result = result.Merge(ForDataInfo(oldDataInfo, afterByID[dataInfoID]));
            }
        ...
        {
            Dictionary<string, DataInfo> beforeByID = (before ?? Enumerable.Empty<DataInfo>()).Where(dataInfo => dataInfo != null).GroupBy(dataInfo => dataInfo.ID, StringComparer.Ordinal).ToDictionary(group => group.Key, group => group.Last(), StringComparer.Ordinal);
            ValidationRequest result = new(ValidationAspect.None);
            foreach (DataInfo dataInfo in after ?? Enumerable.Empty<DataInfo>())
            {
        ...
        {
            Alias[] oldAliases = (before ?? Enumerable.Empty<Alias>()).ToArray();
            Alias[] newAliases = (after ?? Enumerable.Empty<Alias>()).ToArray();
            string[] dataInfoIDs = (dataInfos ?? Enumerable.Empty<DataInfo>()).Where(dataInfo => DataInfoValidationContext.GetSavedSourcePaths(dataInfo).Any(path => ResolutionChanged(path, oldAliases, newAliases))).Select(dataInfo => dataInfo.ID).ToArray();
            string[] patientIDs = (patients ?? Enumerable.Empty<Patient>()).Where(patient => GetSavedAssetPaths(patient).Any(path => ResolutionChanged(path, oldAliases, newAliases))).Select(patient => patient.ID).ToArray();

            ValidationRequest sources = new(dataInfoIDs.Length == 0 ? ValidationAspect.None : ValidationAspect.SourceAvailability, dataInfoIDs: dataInfoIDs, force: true);
        ...
        private static Dictionary<string, string> GetEpochingSignatures(Protocol protocol)
        {
            ...
            return protocol.Blocs.SelectMany(bloc => bloc.SubBlocs).Where(subBloc => !string.IsNullOrEmpty(subBloc.ID)).GroupBy(subBloc => subBloc.ID, StringComparer.Ordinal).ToDictionary(group => group.Key, group =>
            {
                ...
                bool epochable = subBloc.Window.Length > 0 && mainEvent != null && mainEvent.Codes.Count > 0;
        ...
        private static string GetAssetSignature(Patient patient)
        {
            ...
            IEnumerable<string> meshes = patient.Meshes.Select(mesh => mesh switch
            {
            ...
            IEnumerable<string> MRIs = patient.MRIs.Select(MRI => $"{MRI.ID}:{MRI.Name}:{MRI.SavedFile}");
