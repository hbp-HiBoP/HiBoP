...
namespace HBP.Module3D
{
    ...
    public class SinglePatient3DScene : Base3DScene
    {
        ...
        public bool ResetElectrodes(List<string> pathsElectrodesPtsFile, List<string> namePatients)
        {
            ...
            SceneInformation.SitesLoaded = m_Column3DViewManager.DLLLoadedPatientsElectrodes.LoadPTSFiles(pathsElectrodesPtsFile, namePatients, GlobalGOPreloaded.MarsAtlasIndex); // TODO (maybe) : replace with values from visualisation
            ...
            //for (int ii = 0; ii < Patient.Brain.Connectivities.Count; ++ii)
            {
                ...
                {
                    // load latency file
                    latencies = m_Column3DViewManager.DLLLoadedRawSitesList.UpdateLatenciesWithFile(Patient.Brain.PlotsConnectivity);// Connectivities[ii].Path);
                }
