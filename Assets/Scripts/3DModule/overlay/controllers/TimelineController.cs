...
namespace HBP.VISU3D
{
    ...
    public class TimelineController : IndividualSceneOverlayController
    {
        ...
        public new void init(Base3DScene scene, CamerasManager camerasManager)
        {
            ...
            // init global 
            //      init timeline
            m_globalTimelineElements = Instantiate(BaseGameObjects.Timeline);
            ...
            m_globalTimeSlider.onValueChanged.AddListener(
                delegate
                {
                    m_scene.setTime(0, m_globalTimelinePanel.Find("slider panel").Find("value_slider").gameObject.GetComponent<Slider>().value, true);
                });
            ...
            m_globalComputeButton.gameObject.GetComponent<Button>().onClick.AddListener(
                delegate
                {
                    Listeners.computeInfluences(m_scene);
                });
        ...
        public override void updateUI()
        {
            ...
            for (int ii = 0; ii < m_timelineElementsList.Count; ++ii)
            {
                ...
                setTimelineState(timelinePanel, computeButton, currentMode);
            }
        ...
        public override void updateUIPosition()
        {
            ...
            {
                // min position
                if (m_timelinesList[m_currentTimelineID].Min.Position != 0 && m_timelinePanelList.Count > 1)
                {
                    float posMinText = (1f * m_timelinesList[m_currentTimelineID].Min.Position / (m_size - 1));
                ...
                // max position
                if (m_timelinesList[m_currentTimelineID].Max.Position != m_size && m_timelinePanelList.Count > 1)
                {
                    float posMaxText = (1f * m_timelinesList[m_currentTimelineID].Max.Position / (m_size - 1));
            ...
            isEnoughtRoom = (rectCamera.width > 250);
        ...
        public void updateTimelinesUI(List<HBP.Data.Visualisation.TimeLine> timelinesList)
        {
            ...
            for (int ii = 0; ii < m_timelineElementsList.Count; ++ii)
            {
                ...
                slider.minValue = 0;
                slider.maxValue = timeLine.Size - 1;

                m_uniteMin = timeLine.Min.Unite;
                m_uniteMax = timeLine.Max.Unite;

                if (m_min > timeLine.Min.Value)
                    m_min = timeLine.Min.Value;

                if (m_max < timeLine.Max.Value)
                    m_max = timeLine.Max.Value;

                if (posMin > timeLine.Min.Position)
                    posMin = timeLine.Min.Position;

                if (posMax < timeLine.Max.Position)
                    posMax = timeLine.Max.Position;

                m_size = timeLine.Size; // sizes indentical for all columns     
            ...
            for (int ii = 0; ii < m_timelineElementsList.Count; ++ii)
            {
                ...
                // define texts
                string startS = "", endS = "";
                startS += m_min + timeLine.Min.Unite;
                endS += m_max + timeLine.Max.Unite;
                ...
                timelinePanel.Find("min_text").GetComponent<Text>().text = "" + timeLine.Min.Value + m_uniteMin;
                timelinePanel.Find("max_text").GetComponent<Text>().text = "" + timeLine.Max.Value + m_uniteMax;

                // define texture
                m_sliderTexture = UITextureGenerator.getSliderBackgroundTexture(posMin, posMax-1, timeLine.Min.Position, timeLine.Max.Position, timeLine.Size, m_positionMainEvent, m_secondaryEventsPositions[ii]);
            ...
            m_sliderGlobalTexture = UITextureGenerator.getSliderBackgroundTexture(0, m_size - 1, 0, m_size - 1, m_size, m_positionMainEvent, new List<int>());
        ...
        {
            if(m_selectedColumnIsIRMF || m_scene.isLatencyMode() || mode == null)
            {
            ...
            if (mode.m_idMode == Mode.ModesId.AmplitudesComputed) 
            {
                m_timelineIsEnabled = true;
                timelinePanel.gameObject.SetActive(true);
                computeButton.gameObject.SetActive(m_scene.data_.iEEGOutdated);
                textComputeButton = "Update amplitudes...";
                colorComputeButton = Color.red;
            }
            else if (mode.m_idMode == Mode.ModesId.ComputingAmplitudes)
            {
                m_timelineIsEnabled = false;
            ...
            else if (mode.m_idMode == Mode.ModesId.AmpNeedUpdate)
            {
                m_timelineIsEnabled = true;
                computeButton.gameObject.GetComponent<Button>().interactable = true;
                textComputeButton = "Update amplitudes";
                computeButton.gameObject.SetActive(true);
                timelinePanel.gameObject.SetActive(true);
            }
            else if (mode.m_idMode == Mode.ModesId.ROICreation)
            {
                m_timelineIsEnabled = true;
                computeButton.gameObject.GetComponent<Button>().interactable = true;
                textComputeButton = "Update amplitudes";
            ...
            else // amp not computed
            {
                m_timelineIsEnabled = false;
                computeButton.gameObject.GetComponent<Button>().interactable = true;
                textComputeButton = "Compute amplitudes";
                computeButton.gameObject.SetActive(true);
                timelinePanel.gameObject.SetActive(false);
            }
        ...
        public void updateTimelinesNumber(int nbColumns)
        {
            ...
            if (diff < 0) // add timelines
            {
                ...
                {
                    // add timeline
                    GameObject timelineElements = Instantiate(BaseGameObjects.Timeline);
                    ...
                    // set listeners
                    m_timelinePanelList[id].Find("slider panel").Find("value_slider").gameObject.GetComponent<Slider>().onValueChanged.AddListener(
                        delegate
                        {
                            m_scene.setTime(id, m_timelinePanelList[id].Find("slider panel").Find("value_slider").gameObject.GetComponent<Slider>().value, false);
                        });
                    m_computeButtonList[id].gameObject.GetComponent<Button>().onClick.AddListener(delegate
                    {
                        Listeners.computeInfluences(m_scene);
                    });
                    m_timelinePanelList[id].Find("global button").gameObject.GetComponent<Button>().onClick.AddListener(
                        delegate
                        {
        ...
        private void toggleGlobal()
        {
            ...
            m_scene.updateAllTimes(m_showGlobal);
        }
        ...
        private IEnumerator globalLoopTimeline()
        {
            ...
            float startValue = m_globalTimeSlider.minValue;
        ...
        private IEnumerator loopIndividualTimeline(Button loopButton, Slider timelineSlider, int id)
        {
            ...
            float startValue = timelineSlider.minValue;
