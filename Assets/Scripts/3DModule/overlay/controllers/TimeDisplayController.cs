...
namespace HBP.VISU3D
{
    ...
    public class TimeDisplayController : IndividualSceneOverlayController
    {
        ...
        public new void init(Base3DScene scene, CamerasManager camerasManager)
        {
            ...
            columnsTimeDisplay.Add(Instantiate(BaseGameObjects.TimeDisplay));
        ...
        public void updateTime(int id, float value, bool global)
        {
            ...
            int size = m_iEEGTimelines[0].Size;
            string uniteMin = "";
            ...
            if (global)
            {
                ...
                {
                    uniteMin = m_iEEGTimelines[ii].Min.Unite;

                    if (min > m_iEEGTimelines[ii].Min.Value)
                        min = m_iEEGTimelines[ii].Min.Value;

                    if (max < m_iEEGTimelines[ii].Max.Value)
                        max = m_iEEGTimelines[ii].Max.Value;
                }
            ...
            {
                min = m_iEEGTimelines[id].Min.Value;
                max = m_iEEGTimelines[id].Max.Value;
                uniteMin = m_iEEGTimelines[id].Min.Unite;
        ...
        public override void updateUIPosition()
        {
            for (int ii = 0; ii < columnsTimeDisplay.Count; ++ii)
            {
                ...
                if (!currentCamera.isMinimized())
                {
                    ...
                    bool previousRoom = isEnoughtRoom;
                    isEnoughtRoom = (rectCamera.width > 250) && (rectCamera.height > 300);

                    if (m_minimized[ii] != previousMin || previousRoom != isEnoughtRoom)
                    {
                        updateUI();
                    }
                }
