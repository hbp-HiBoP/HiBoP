...
namespace HBP.VISU3D
{
    ...
    public class MP3DScene : Base3DScene
    {
        ...
        public bool reset_electrodes_files(List<string> pathsElectrodesPtsFile, List<string> names)
        {
            ...
            for (int ii = 0; ii < m_CM.IEEG_columns_nb(); ++ii)
            {
                m_CM.IEEG_col(ii).id_selected_site = -1;
            }
        ...
        public void load_patient_in_SP_scene(int idPatientSelected, int idPlotSelected)
        {
            ...
            int size = m_CM.IEEG_col(0).columnData.Values[0].Length;
            ...
            List<Data.Visualisation.ColumnData> columnDataList = new List<Data.Visualisation.ColumnData>(m_CM.IEEG_columns_nb());
            for (int ii = 0; ii < m_CM.IEEG_columns_nb(); ++ii)
            {
                Data.Visualisation.ColumnData columnData = new Data.Visualisation.ColumnData();
                columnData.Label = m_CM.IEEG_col(ii).Label;

                // copy iconic scenario reference
                columnData.IconicScenario = m_CM.IEEG_col(ii).columnData.IconicScenario;

                // copy timeline reference
                columnData.TimeLine = m_CM.IEEG_col(ii).columnData.TimeLine;

                // fill new mask
                columnData.PlotMask = new bool[nbPlotsSpPatient];
                for (int jj = 0; jj < nbPlotsSpPatient; ++jj)
                {
                    columnData.PlotMask[jj] = m_CM.IEEG_col(ii).columnData.PlotMask[startId + jj];
                }
                ...
                for (int jj = 0; jj < nbPlotsSpPatient; ++jj)
                {
                    ...
                    {
                        valuesT[kk] = m_CM.IEEG_col(ii).columnData.Values[startId + jj][kk];
                    }
            ...
            List<List<bool>> blackListMask = new List<List<bool>>(m_CM.IEEG_columns_nb());
            List<List<bool>> excludedMask = new List<List<bool>>(m_CM.IEEG_columns_nb());
            List<List<bool>> hightLightedMask = new List<List<bool>>(m_CM.IEEG_columns_nb());
            for (int ii = 0; ii < m_CM.IEEG_columns_nb(); ++ii)
            {
                ...
                for (int jj = 0; jj < nbPlotsSpPatient; ++jj)
                {
                    blackListMask[ii].Add(m_CM.IEEG_col(ii).Sites[startId + jj].blackList);
                    excludedMask[ii].Add(m_CM.IEEG_col(ii).Sites[startId + jj].exclude);
                    hightLightedMask[ii].Add(m_CM.IEEG_col(ii).Sites[startId + jj].highlight);
                }
            }
        ...
        private void change_size_ROI(float coeff)
        {
            ...
            {
                ROI ROI = m_CM.is_FMRI_column(idC) ? m_CM.FMRI_col(idC - m_CM.IEEG_columns_nb()).ROI : m_CM.IEEG_col(idC).ROI;
                if (ROI == null)
                    return;
            ...
            switch (keyCode)
            {
                ...
                case KeyCode.Delete:                
                    if (data_.ROICreationMode)
                    {
                        ...
                        Column3DView col =  m_CM.current_column();                        
        ...
        public void click_on_scene(Ray ray)
        {
            ...
            if(plotHit) // plot collision -> select PLOT
            {
                ...
                m_CM.current_column().id_selected_site = idPlotGlobal;
            ...
            else
            {
                if (data_.ROICreationMode)
                {
                    ...
                    {
                        if (m_CM.current_column().ROI.check_collision(ray))
                        {
                            int idClickedBubble = m_CM.current_column().ROI.collided_closest_bubble_id(ray);
                            SelectBubbleEvent.Invoke(m_CM.idSelectedColumn, idClickedBubble);
                        }
        ...
        public new void move_mouse_on_scene(Ray ray, Vector3 mousePosition, int idColumn)
        {
            ...
            UpdateDisplayedSitesInfo.Invoke(new SiteInfo(site, true, mousePosition, m_CM.is_current_column_FMRI(), false, hits[idHitToKeep].collider.GetComponent<Site>().fullName, "" + amp));
        ...
        public override void send_additionnal_site_info_request(Site previousPlot = null) // TODO deporter dans c manager
        {
            ...
            if (currIEEGCol.id_selected_site == -1)
                return;

            string[] elements = m_CM.SitesList[currIEEGCol.id_selected_site].GetComponent<Site>().fullName.Split('_');
            ...
            for (int ii = 0; ii < m_CM.IEEG_columns_nb(); ++ii)
            {
                ...
                bool isROI = (m_CM.IEEG_col(ii).ROI.bubbles_nb() > 0);
            ...
            request.idSite1 = currIEEGCol.id_selected_site;
        ...
        public void update_current_ROI(int idColumn)
        {
            ...
            for (int ii = 0; ii < maskROI.Length; ++ii)
                maskROI[ii] = m_CM.col(idColumn).Sites[ii].columnROI;

            m_CM.col(idColumn).ROI.update_mask(m_CM.col(idColumn).RawElectrodes, maskROI);
            for (int ii = 0; ii < m_CM.col(idColumn).Sites.Count; ++ii)
                m_CM.col(idColumn).Sites[ii].columnROI = maskROI[ii];
        ...
        {
            m_CM.col(idColumn).update_ROI(roi);
        ...
        {
            Column3DView currentCol = m_CM.current_column();
            return "ROI :\n" +  currentCol.ROI.ROIbubbulesInfos() + currentCol.site_state_str();
        }
        ...
        {
            Column3DView currentCol = m_CM.current_column();
            return "Sites in ROI:\n" + currentCol.ROI.ROIbubbulesInfos() + currentCol.only_sites_in_ROI_str();
        }
        ...
        public void update_sites_mask(int idColumn, List<List<List<SiteI>>> plots, List<string> patientsName)
        {
            ...
            for (int ii = 0; ii < CM.col(idColumn).Sites.Count; ++ii)
            {
                CM.col(idColumn).Sites[ii].exclude = false;
                CM.col(idColumn).Sites[ii].blackList = false;
                CM.col(idColumn).Sites[ii].highlight = false;
                CM.col(idColumn).Sites[ii].columnMask = false;
            }
            ...
            for (int ii = 0; ii < CM.col(idColumn).Sites.Count; ii++)
            {
                ...
                {
                    if (patientsName[jj] != CM.col(idColumn).Sites[ii].patientName)
                        continue;                    
                    ...
                    for (int kk = 0; kk < plots[jj].Count; kk++) // electrode
                    {
                        for(int ll = 0; ll < plots[jj][kk].Count; ll++) // plot
                        {
                            ...
                            if (namePlot != CM.col(idColumn).Sites[ii].fullName)
                                continue;

                            CM.col(idColumn).Sites[ii].exclude = plots[jj][kk][ll].exclude;
                            CM.col(idColumn).Sites[ii].blackList = plots[jj][kk][ll].blackList;
                            CM.col(idColumn).Sites[ii].highlight = plots[jj][kk][ll].highlight;
                            CM.col(idColumn).Sites[ii].columnMask = plots[jj][kk][ll].columnMask;
                        }
                    }
