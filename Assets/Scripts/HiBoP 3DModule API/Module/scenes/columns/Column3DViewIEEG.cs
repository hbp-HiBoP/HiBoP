...
namespace HBP.VISU3D
{
    ...
    public class Column3DViewIEEG : Column3DView
    {
        ...
        public new void init(int idColumn, int nbCuts, DLL.PatientElectrodesList plots, List<GameObject> PlotsPatientParent)
        {
            // call parent init
            base.init(idColumn, nbCuts, plots, PlotsPatientParent);
            isFMRI = false;
        ...
        {
            bool noROI = spScene ? false : (m_ROI.bubbles_nb() == 0);
        ...
        public void update_sites_rendering(SceneStatesInfo data, Latencies latenciesFile)
        {
            ...
            else // iEEG
            {
                ...
                if (id_selected_site >= 0 && id_selected_site < Sites.Count)
                    m_selectRing.set_selected_site(Sites[id_selected_site], Sites[id_selected_site].transform.localScale);


                UnityEngine.Profiling.Profiler.EndSample();
            }

            if (id_selected_site == -1)
            {
