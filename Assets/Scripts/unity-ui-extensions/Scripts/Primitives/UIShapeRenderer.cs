...
{
    [AddComponentMenu("UI/Extensions/Primitives/UIShapeRenderer")]
    public class UIShapeRenderer : UIPrimitiveBase
    {
        ...
        [SerializeField]
        private float[] m_shapeThickness;
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            for (var i = 1; i < pointsToDraw.Length; i++)
            {
                ...
                var startThickness = m_shapeThickness[i - 1];
                var endThickness = m_shapeThickness[i];
                start = new Vector2(start.x * sizeX + offsetX, start.y * sizeY + offsetY);
                end = new Vector2(end.x * sizeX + offsetX, end.y * sizeY + offsetY);
                segments.Add(CreateShapeSegment(start, end,startThickness,endThickness, SegmentType.Middle));
            }
            ...
            for (var i = 0; i < segments.Count; i++)
            {
                if (i < segments.Count - 1)
                {
                    ...
                    var miterDistance = m_shapeThickness[i + 1] / (2 * Mathf.Tan(angle / 2));
        ...
        private UIVertex[] CreateShapeSegment(Vector2 start, Vector2 end, float startThickness, float endThickness, SegmentType type)
        {
            var uvs = middleUvs;
            ...
            var v1 = start - offsetStart;
            var v2 = start + offsetStart;
            var v3 = end + offsetEnd;
            var v4 = end - offsetEnd;
