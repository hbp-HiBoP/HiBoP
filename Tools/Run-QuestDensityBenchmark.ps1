...
param(
    ...
    [string]$Apk = "$PSScriptRoot/../.artifacts/quest-019/Android/HiBoP.Quest.apk",
...
$run = "$repo/.test-results/quest-019/benchmark-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss'))"
...
try {
    ...
    Invoke-Adb @('install','-r',$Apk) | Set-Content "$run/install.log"
    ...
    Invoke-Adb @('push',$Fixture,"$remote/quest019.hbna") | Out-Null
    [IO.File]::WriteAllText("$run/marker", [Guid]::NewGuid().ToString('N'))
    Invoke-Adb @('push',"$run/marker","$remote/quest019-benchmark") | Out-Null
    # Remove only the completion files belonging to this diagnostic; stale completion cannot pass.
    Invoke-Adb @('shell','rm','-f',"$remote/quest019-benchmark-result/complete.json","$remote/quest019-benchmark-result/failure.txt") | Out-Null
    ...
    do {
        ...
        $lines = @(Invoke-Adb @('logcat','-d','-v','brief',"--pid=$densityPid"))
        $lines | Set-Content "$run/quest.log"
        if ($lines -match 'QUEST019_BENCHMARK_FAIL') { throw "Benchmark failed: $run/quest.log" }
        if ([DateTime]::UtcNow -gt $deadline) { throw "Benchmark timed out: $run/quest.log" }
    } until ($lines -match 'QUEST019_BENCHMARK_COMPLETE')
    Invoke-Adb @('pull',"$remote/quest019-benchmark-result","$run/Android") | Out-Null
    ...
    @{fixture=$Fixture; fixtureSha256=(Get-FileHash $Fixture).Hash; apk=$Apk; apkSha256=$apkHash; installedSha256=$installedHash; model=$model; completed=$true} | ConvertTo-Json | Set-Content "$run/result.json"
