...
$run = "$repo/.test-results/quest-023/delivery-$runId"
New-Item -ItemType Directory -Force -Path $run | Out-Null
$serverConfig = "$run/server-private.json"
$clientConfig = "$run/client-private.json"
$player = "$repo/.artifacts/quest-023/Windows/HiBoP.6.1.0.win64/HiBoP.exe"
...
try {
    ...
    $arguments = @('-questIEEGProtocol',"`"$repo/.artifacts/quest-020/fixture/quest-020.prov`"",'-pf',"`"$fixture`"",'-v','"MNI iEEG"','-screen-fullscreen','0','-ieegDelivery',"`"$serverConfig`"",'-logFile',"`"$run/windows.log`"")
    ...
    Invoke-Adb @('shell','rm','-f',"$remote/quest023-received","$remote/quest023-delivery-result.json") | Out-Null
    Invoke-Adb @('push',$clientConfig,"$remote/quest023-delivery.json") | Out-Null
    ...
    do {
        ...
        & $AdbPath -s $Serial shell test -f "$remote/quest023-received"
        $received = $LASTEXITCODE -eq 0
        if ([DateTime]::UtcNow -gt $deadline) { throw 'Quest did not publish iEEG after reception.' }
    } until ($received)
    ...
    $radioScript = "/data/local/tmp/quest023-$runId.sh"
    $radioLog = "/data/local/tmp/quest023-$runId.log"
    $script = @'
#!/system/bin/sh
set -e
...
'@
    ...
    if ($Serial -match ':') { & $AdbPath connect $Serial | Out-Null }
    Invoke-Adb @('pull',$radioLog,"$run/radio.log") | Out-Null
    Invoke-Adb @('pull',"$remote/quest023-delivery-result.json","$run/device-result.json") | Out-Null
    ...
    if ((@(Invoke-Adb @('shell','pidof',$package)) -join '').Trim() -ne $ieegPid) { throw 'Quest process changed during outage.' }
    ...
    Invoke-Adb @('logcat','-d','-v','threadtime',"--pid=$ieegPid") | Set-Content "$run/quest.log"
    @{passed=$true; player=$player; playerSha256=(Get-FileHash $player).Hash; contentHash=$ready.hash; processId=$ieegPid; ownerValidation='PENDING'} | ConvertTo-Json | Set-Content "$run/result.json"
    Write-Output "Offline iEEG verified: $run. Quest remains open for manual validation."
}
finally {
