...
foreach ($path in @($player, $fixture, $protocol)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Missing $path. Build $EvidenceId Windows and run Tools/Prepare-QuestIEEGFixture.py first." }
}
$run = "$repo/.test-results/$EvidenceId/player-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss'))"
