...
**FAIL P12 du 7 septembre 2026.** Le pipeline de coupe exact/latest-wins est implémenté, mais une mesure optimiste sans calcul Desktop, upload GPU ni attente de frame dépasse déjà les seuils. Pour D2 en 512², le codec hôte p95 vaut `169,007 / 445,565 ms` pour 1/3 colonnes et la borne réseau au meilleur débit P06 vaut `215,845 / 647,454 ms`. D4 8×2048² produit `134 218 280` octets, `17 811,200 ms` p95 de codec hôte et `27 621,903 ms` de borne réseau. P12 est NO-GO production et requiert la décision P12-E ; PX2 et `hbp_core` Quest restent interdits sans autorisation explicite. Voir la [fiche P12/D20](evidence/P12/P12-D20-fail-decision.md).
