...
Every encoded body above the measured inline threshold uses a bulk stream, regardless of operation family. This includes job/checkpoint data, large D33/D34 configuration/site batches and dense triangle masks. The small descriptor remains in the scene-operation stream and declares operation/job/checkpoint identity, generation when applicable, touched keys and barrier scope, reliable bulk-stream identity, total length, chunk count/size bounds, content digest and body schema. Bulk stream IDs encode a per-session, per-origin 64-bit ordinal. Each peer advertises the contiguous bulk ordinal it has retired in `ResumeHello` and advances it during a live connection with `BulkStreamRetirement`. A retirement claim means every stream through that ordinal is complete or cancelled and will not be replayed; receivers then discard those stream watermarks. This lets duplicate data and ACKs for retired streams be ignored without retaining unbounded GUID histories. At most 126 bulk ordinals may remain unretired per direction, keeping the 128-entry resume table bounded with the two permanent streams; optional bulk admission backpressures at that window and required bulk admission faults the session. Chunks may be interleaved with interactive messages. The receiver writes into a bounded detached buffer/stream and validates completion before one atomic Unity apply. Independent touched keys may continue while the body is incomplete; overlapping keys wait behind the descriptor barrier.
...
Within the 500 ms grace, reconnect proves the same session/incarnations and exchanges each active reliable stream's cumulative receive watermark plus each direction's contiguous bulk retirement watermark, origin sequences and accepted per-key/checkpoint identity. Each stream replays every unacknowledged frame with unchanged IDs and sequences; within that stream, replay precedes newer sequence values. A peer's retirement watermark proves the receiver has already acknowledged those streams, so their old per-stream receive watermarks can be safely retired.
