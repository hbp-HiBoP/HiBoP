...
\maketitle
\begin{abstract}
This document is the user manual of the Human Intracranial Brain Observation Player (HiBoP). HiBoP is being developed by the Centre de Recherche en Neurosciences de Lyon (CRNL), with funds from the Human Brain Project (HBP) to visualize large datasets of intracranial electroencephalography (iEEG) and functional MRI data. This document has been written for the 4.3.2 version of HiBoP.
\end{abstract}
\tableofcontents
...
\caption{Project directory hierarchy}\label{projectDirectory}
\end{figure}
\subsection{Settings}
...
\paragraph{} A datainfo can be of type iEEG, CCEP, fMRI, shared fMRI, MEGc, MEGv or static. In the case of iEEG, you can specify a normalization method (see Section \ref{preferences} for detailed information about normalization). In the case of CCEP, you have to specify the channel that has been stimulated. MEGc corresponds to MEG data represented as time series (similar to iEEG) and MEGv corresponds to MEG data represented in voxels (similar to fMRI). Shared fMRI is a fMRI not linked to a specific patient. Static data is channeled data with one float value per label per channel stored in a csv file. Each datainfo contains a data container which handles the data files.
\paragraph{} A data container can be of type ELAN\footnote{\label{ELAN}ELAN format – ELAN is freely available at http://elan.lyon.inserm.fr}, Micromed, BrainVision, EDF, NIFTI-1, FIF or csv. Each datainfo type can handle some of these data types:
\begin{itemize}
\item \textbf{iEEG}: ELAN, Micromed, BrainVision, EDF
\item \textbf{CCEP}: ELAN, Micromed, BrainVision, EDF
\item \textbf{fMRI}: NIFTI-1
\item \textbf{Shared fMRI}: NIFTI-1
\item \textbf{MEGc}: BrainVision, EDF, FIF
\item \textbf{MEGv}: NIFTI-1
\item \textbf{Static}: csv
\end{itemize}
...
\end{center}
\caption{User Preferences}\label{preferencesUI}
\end{figure}
\begin{itemize}
\item \textbf{General}
\begin{itemize}
...
\item \textbf{Data}
\begin{itemize}
\item \textbf{Enable automatic activity update}: Triggers the update of the brain activity on the brain each time a parameter is changed in the visualization
\item \textbf{Raw MRI cuts}: Does not smoothen the cuts and shows the whole cut square in the 3D visualization
...
\end{center}
\caption{Create a new project}\label{newProjectUI}
\end{figure}
...
\end{center}
\caption{Open a project}\label{openProjectUI}
\end{figure}
...
\end{center}
\caption{Save a project as}\label{saveProjectUI}
\end{figure}
...
\end{center}
\caption{Patients manager}\label{patientGestionUI}
\end{figure}
...
\end{center}
\caption{Patient modifier}\label{patientModifierUI}
\end{figure}
...
\end{center}
\caption{Groups manager}\label{groupGestionUI}
\end{figure}
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{GroupModifier.png}
\end{center}
\caption{Group modifier}\label{groupModifierUI}
\end{figure}
...
\end{center}
\caption{Protocols manager}\label{protocolGestionUI}
\end{figure}
...
\end{center}
\caption{Protocol modifier}\label{protocolModifierUI}
\end{figure}
...
\end{center}
\caption{Bloc modifier}\label{blocModifierUI}
\end{figure}
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{SubBlocModifier.png}
\end{center}
\caption{SubBloc modifier}\label{subBlocModifierUI}
\end{figure}
...
\end{center}
\caption{Datasets manager}\label{datasetGestionUI}
\end{figure}
...
\end{center}
\caption{Dataset modifier}\label{datasetModifierUI}
\end{figure}
...
\end{center}
\caption{Data information modifier}\label{dataInfoModifierUI}
\end{figure}
...
\end{center}
\caption{Visualizations manager}\label{visuGestionUI}
\end{figure}
...
\end{center}
\caption{Visualization modifier}\label{visuModifierUI}
\end{figure}
...
\end{center}
\caption{HiBoP user interface}\label{globalUI}
\end{figure}
...
\end{center}
\caption{Cut parameters panel}\label{cut}
\end{figure}
...
\end{center}
\caption{Sites list panel}\label{sitesListPanel}
\end{figure}
...
\end{center}
\caption{"Filter sites" submenu}\label{basicStates}
\end{figure}
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{SitesAction.png}
\end{center}
\caption{"Apply action on filtered sites" submenu}\label{sitesAction}
\end{figure}
...
\end{center}
\caption{Graph for a protocol and three blocs (site U'8)}\label{graph}
\end{figure}
...
\end{center}
\caption{Trial matrix of a site}\label{trialMatrix}
\end{figure}
...
\end{center}
\caption{Example graphs grid}\label{graphsGrid}
\end{figure}
...
\item \textbf{From directory}: this is almost the same as "From database", except it only takes one specific directory into account, allowing to import only one object instead of loading a whole database and then selecting which objects you want to import.
\end{itemize}
